...
public class GraphSearch extends Configured implements Tool {
    ...
    public void map(LongWritable key, Text value, OutputCollector<IntWritable, Text> output,
      ...
      if (node.getColor() == Node.Color.GRAY) {
        for (int v : node.getEdges()) {
          ...
          vnode.setDistance(node.getDistance() + weight);
  ...
  public int run(String[] args) throws Exception {
	...
	for(int i = 0; i < args.length; ++i){
		if("-i".equals(args[i])){
			totalC = Integer.parseInt(args[++i]);
		}
	}
	
    while (keepGoing(iterationCount,totalC)) {
  ...
  private boolean keepGoing(int iterationCount, int totalC) {
    if(iterationCount >= totalC) {
      return false;
    }
